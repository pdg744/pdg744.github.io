import { useCallback, useState } from "react";
import * as Router from "expo-router";
import {
  Image,
  Pressable,
  ScrollView,
  StyleSheet,
  Text,
  View,
} from "react-native";
import * as Svg from "react-native-svg";
import * as SafeArea from "react-native-safe-area-context";
import * as Theme from "../constants/theme.js";
import logoAsset from "../assets/logo-mark.png";
import HomeControls from "../components/HomeControls.jsx";
import PracticeSummary from "../components/PracticeSummary.jsx";
import { useFeatureSettings } from "../hooks/useFeatureSettings.js";
function DiffyIcon({ size = 48 }) {
  return (
    <Svg.default width={size} height={size} viewBox={"0 0 100 100"}>
      <Svg.Polygon
        points={"8,8 92,8 92,92 8,92"}
        fill={"none"}
        stroke={Theme.Colors.orange}
        strokeWidth={"3"}
        strokeLinejoin={"round"}
      />
      <Svg.Polygon
        points={"50,8 92,50 50,92 8,50"}
        fill={"none"}
        stroke={Theme.Colors.teal}
        strokeWidth={"2.5"}
        strokeLinejoin={"round"}
      />
      <Svg.Polygon
        points={"71,29 71,71 29,71 29,29"}
        fill={"none"}
        stroke={Theme.Colors.gold}
        strokeWidth={"2"}
        strokeLinejoin={"round"}
      />
      <Svg.Polygon
        points={"71,50 50,71 29,50 50,29"}
        fill={"none"}
        stroke={Theme.Colors.darkTeal}
        strokeWidth={"1.75"}
        strokeLinejoin={"round"}
      />
      <Svg.Polygon
        points={"60.5,60.5 39.5,60.5 39.5,39.5 60.5,39.5"}
        fill={"none"}
        stroke={Theme.Colors.deepOrange}
        strokeWidth={"1.5"}
        strokeLinejoin={"round"}
      />
    </Svg.default>
  );
}
function FactorGraphIcon({ size = 64 }) {
  const nodes = [
    [18, 12],
    [48, 12],
    [78, 34],
    [33, 42],
    [33, 66],
    [78, 66],
    [55, 90],
  ];
  return (
    <Svg.default
      width={size}
      height={size}
      viewBox="0 0 100 100"
      accessible={false}
    >
      <Svg.Path
        d="M18 12 L33 42 L33 66 L55 90 M48 12 L33 42 M78 34 L78 66 L55 90"
        fill="none"
        stroke={Theme.Colors.teal}
        strokeWidth={2}
        strokeLinejoin="round"
      />
      {nodes.map(([x, y]) => (
        <Svg.Circle
          key={`${x}-${y}`}
          cx={x}
          cy={y}
          r={6}
          fill={Theme.Colors.surface}
          stroke={Theme.Colors.teal}
          strokeWidth={2}
        />
      ))}
    </Svg.default>
  );
}
const activities = [
  {
    id: "diffy-squares",
    label: "Diffy Squares",
    icon: "diffy-squares",
    color: Theme.Colors.orange,
  },
  {
    id: "factor-and-add",
    label: "Factor and Add",
    icon: "factor-and-add",
    color: Theme.Colors.teal,
  },
];
function ActivitiesHome() {
  const [menuOpen, setMenuOpen] = useState(false);
  Router.useFocusEffect(useCallback(() => { setMenuOpen(false); }, []));
  const { stars } = useFeatureSettings();
  const router = Router.useRouter();
  return (
    <SafeArea.SafeAreaView style={styles.container}>
      <ScrollView contentContainerStyle={styles.grid}>
      <View style={styles.header}>
        <View style={styles.headerTop}>
          <Pressable
            accessibilityRole="button"
            accessibilityLabel="Math Explorers menu"
            accessibilityState={{ expanded: menuOpen }}
            onPress={() => setMenuOpen((open) => !open)}
            style={styles.logoButton}
          >
            <Image source={logoAsset} style={styles.logo} accessible={false} />
            <Text style={styles.chevron}>{menuOpen ? "⌃" : "⌄"}</Text>
          </Pressable>

        </View>
        {menuOpen && <HomeControls />}
      </View>
        {stars && <PracticeSummary initialDays={7} onSelect={(type, days) => router.push({ pathname: "/practice-details", params: { type, days: days ?? "all" } })} />}
        {activities.map((o) => (
          <Pressable
            accessibilityRole="button"
            style={({ pressed: e }) => [
              styles.card,
              {
                borderColor: o.color,
              },
              e && styles.cardPressed,
            ]}
            onPress={() => router.push(`/${o.id}`)}
            key={o.id}
          >
            <View style={styles.cardRow}>
              {"diffy-squares" === o.icon ? (
                <View style={styles.cardIconWrapper}>
                  <DiffyIcon size={64} />
                </View>
              ) : (
                <View style={styles.cardIconWrapper}>
                  <FactorGraphIcon size={64} />
                </View>
              )}
              <View style={styles.cardText}>
                <Text
                  style={[
                    styles.cardLabel,
                    {
                      color: o.color,
                    },
                  ]}
                  numberOfLines={1}
                  adjustsFontSizeToFit={true}
                >
                  {o.label}
                </Text>
              </View>
            </View>
          </Pressable>
        ))}
      </ScrollView>
    </SafeArea.SafeAreaView>
  );
}
const styles = StyleSheet.create({
  container: {
    flex: 1,
    backgroundColor: Theme.Colors.background,
  },
  header: {
    width: "100%",
    maxWidth: 720,
    alignSelf: "center",
    paddingTop: 8,
    paddingBottom: 8,
  },
  headerTop: {
    flexDirection: "row",
    justifyContent: "flex-end",
    alignItems: "center",
  },
  logo: {
    width: 58,
    height: 58,
  },
  logoButton: { minHeight: 58, flexDirection: "row", alignItems: "center", gap: 8 },
  chevron: { color: Theme.Colors.teal, fontSize: 22 },
  grid: {
    width: "100%",
    maxWidth: 720,
    alignSelf: "center",
    paddingHorizontal: 24,
    gap: 16,
    paddingBottom: 32,
  },
  card: {
    backgroundColor: Theme.Colors.surface,
    borderRadius: 16,
    borderWidth: 1.5,
    padding: 24,
  },
  cardPressed: {
    opacity: 0.75,
    transform: [
      {
        scale: 0.98,
      },
    ],
  },
  cardRow: {
    flexDirection: "row",
    alignItems: "center",
    gap: 16,
  },
  cardIconWrapper: {},
  cardText: {
    flex: 1,
    gap: 4,
  },
  cardLabel: {
    fontSize: 22,
    fontWeight: "800",
    letterSpacing: 0.5,
  },
  cardDescription: {
    color: Theme.Colors.textSecondary,
    fontSize: 14,
    fontWeight: "400",
  },
});
export default ActivitiesHome;
